...
public class CertHttpUtil {
    ...
    public static String postData(String url, String xmlObj, String mchId, String certPath) {
        ...
        try {
            ...
            e.printStackTrace();
            throw new BusinessException("初始化证书失败"+e.getMessage());
        }
        String result = null;
        ...
        try {
            ...
            try {
                ...
                e.printStackTrace();
                throw new BusinessException("发送请求失败"+e.getMessage());
            }
    ...
    private static void initCert(String mchId, String certPath) throws Exception {
        ...
        if (inputStream == null) {
            ...
            //throw new BusinessException("转化流错误");
        }
        SSLContext sslContext;
        try {
            KeyStore keystore = KeyStore.getInstance("PKCS12");
            char[] partnerId2charArray = key.toCharArray();
            keystore.load(inputStream, partnerId2charArray);
            sslContext = org.apache.http.ssl.SSLContexts.custom().loadKeyMaterial(keystore, partnerId2charArray).build();
        } catch (Exception var11) {
            throw new BusinessException("证书文件有问题，请核实！", var11.getMessage());
        } finally {
            inputStream.close();
        }

        SSLConnectionSocketFactory connectionSocketFactory = new SSLConnectionSocketFactory(sslContext, new String[]{"TLSv1"}, null, new DefaultHostnameVerifier());
